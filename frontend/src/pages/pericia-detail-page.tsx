  ...
  return (
    <>
      <DomainPageTemplate
        ...
        headerActions={
          <>
            <button className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm" onClick={() => { setDates({ dataNomeacao: toDateInput(detail.dataNomeacao), dataAgendamento: toDateInput(detail.dataAgendamento), dataRealizacao: toDateInput(detail.dataRealizacao), dataEnvioLaudo: toDateInput(detail.dataEnvioLaudo) }); setShowDatesModal(true); }} type="button"><Pencil size={14} /> Editar Datas</button>
    ...
    <div className="space-y-4">
      <section className="rounded-xl border bg-white p-4 shadow-sm">
        ...
        <section className="rounded-lg border bg-card shadow-sm">
          ...
          <div className="p-4">
            {activeTab === 'Visão 360°' && <div className="grid gap-4 md:grid-cols-2"><div className="rounded-lg border p-4"><h3 className="font-semibold">Resumo do Caso</h3><ul className="mt-3 space-y-2 text-sm text-muted-foreground"><li className="flex justify-between"><span>Status Atual</span><strong>{detail.status?.nome ?? '—'}</strong></li><li className="flex justify-between"><span>Cidade / Vara</span><span>{detail.cidade?.nome ?? '—'} / {detail.vara?.nome ?? '—'}</span></li><li className="flex justify-between"><span>Data Nomeação</span><span>{toDateBR(detail.dataNomeacao)}</span></li></ul></div><div className="rounded-lg border p-4"><h3 className="font-semibold">Prazos</h3><ul className="mt-3 space-y-2 text-sm text-muted-foreground"><li className="flex justify-between"><span>Agendamento</span><span>{toDateBR(detail.dataAgendamento)}</span></li><li className="flex justify-between"><span>Realização</span><span>{toDateBR(detail.dataRealizacao)}</span></li><li className="flex justify-between"><span>Envio Laudo</span><span>{toDateBR(detail.dataEnvioLaudo)}</span></li></ul></div></div>}
            {activeTab === 'Documentos' && <div className="space-y-2">{documentsQuery.isLoading ? <LoadingState /> : (documentsQuery.data ?? []).map((doc) => <div className="rounded-md border p-3 text-sm" key={doc.id}><p className="font-semibold">{doc.nome}</p><p className="text-muted-foreground">Categoria: {doc.categoria ?? '—'} • Tipo: {doc.tipo ?? '—'}</p></div>)}{(documentsQuery.data ?? []).length === 0 && !documentsQuery.isLoading && <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">Nenhum documento anexado.</div>}</div>}
      ...
      {showDatesModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><div className="w-full max-w-md rounded-xl bg-white shadow-xl"><div className="flex items-center justify-between rounded-t-xl bg-slate-900 px-4 py-3 text-white"><p className="font-semibold">Editar Datas / Marcos</p><button onClick={() => setShowDatesModal(false)} type="button">×</button></div><div className="space-y-3 p-4">{[{ key: 'dataNomeacao', label: 'Data Nomeação' }, { key: 'dataAgendamento', label: 'Data Agendamento' }, { key: 'dataRealizacao', label: 'Data Realização' }, { key: 'dataEnvioLaudo', label: 'Data Envio Laudo' }].map((field) => <label className="block text-sm" key={field.key}><span className="mb-1 block text-xs font-semibold uppercase text-muted-foreground">{field.label}</span><div className="relative"><input className="w-full rounded-md border px-3 py-2" onChange={(e) => setDates((prev) => ({ ...prev, [field.key]: e.target.value }))} type="date" value={dates[field.key as keyof typeof dates]} /><CalendarDays className="absolute right-3 top-2.5 text-slate-400" size={16} /></div></label>)}<div className="flex justify-end gap-2 pt-2"><button className="rounded-md px-3 py-2 text-sm" onClick={() => setShowDatesModal(false)} type="button">Cancelar</button><button className="inline-flex items-center gap-1 rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white" disabled={updateDates.isPending} onClick={async () => { await updateDates.mutateAsync({ dataNomeacao: dates.dataNomeacao || undefined, dataAgendamento: dates.dataAgendamento || undefined, dataRealizacao: dates.dataRealizacao || undefined, dataEnvioLaudo: dates.dataEnvioLaudo || undefined }); setShowDatesModal(false); }} type="button"><Save size={14} />Salvar</button></div></div></div></div>}
      {showLaudoModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><div className="w-full max-w-md rounded-xl bg-white shadow-xl"><div className="flex items-center justify-between rounded-t-xl bg-slate-900 px-4 py-3 text-white"><p className="font-semibold">Registrar Protocolo de Laudo</p><button onClick={() => setShowLaudoModal(false)} type="button">×</button></div><div className="space-y-3 p-4"><label className="block text-sm"><span className="mb-1 block text-xs font-semibold uppercase text-muted-foreground">Data Protocolo</span><input className="w-full rounded-md border px-3 py-2" onChange={(e) => setDataProtocoloLaudo(e.target.value)} type="date" value={dataProtocoloLaudo} /></label><div className="flex justify-end gap-2 pt-2"><button className="rounded-md px-3 py-2 text-sm" onClick={() => setShowLaudoModal(false)} type="button">Cancelar</button><button className="inline-flex items-center gap-1 rounded-md bg-teal-700 px-3 py-2 text-sm font-semibold text-white" onClick={() => setShowLaudoModal(false)} type="button"><Send size={14} />Confirmar envio</button></div></div></div></div>}
    </>
  );
};
